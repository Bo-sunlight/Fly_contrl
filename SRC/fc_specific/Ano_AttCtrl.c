...
void Att_2level_Ctrl(float dT_s,s16 *CH_N)
{
	...
	/*摇杆量转换为YAW期望角速度*/
	set_yaw_av_tmp = (s32)(0.0023f *my_deadzone(CH_N[CH_YAW],0,65) *max_yaw_speed) + (-program_ctrl.yaw_pal_dps);
...
void Att_1level_Ctrl(float dT_s)
{
	...
	/*反馈角速度赋值*/
	att_1l_ct.fb_angular_velocity[ROL] =  sensor.Gyro_deg_lpf[X];
	att_1l_ct.fb_angular_velocity[PIT] = -sensor.Gyro_deg_lpf[Y];
	att_1l_ct.fb_angular_velocity[YAW] = -sensor.Gyro_deg_lpf[Z];
